 ...
 */
/**
 * Provides GUI for the Elevators application. The "View" part of the MVC
 * application. Receives action events from the application timer (clock) which
 * generates the events at a predefined rate controlled with the special JSlider
 * gauge. On each action event (time step), the ElevatorGUI object calls the
 * ElevatorGraphics object to perform an animation step, i.e. to inspect the
 * status of the Model (all elevators: motors, doors and scales) and to display
 * a new state (by repainting the ElevatorCanvas) if the state has changed since
 * the previous step.
 */
public class ElevatorGUI extends JFrame implements ActionListener, ChangeListener {
    ...
    /**
     * The application timer services as a clock that generates action events at
     * a predefined rate controlled with the JSlider gauge. On each action event
     * (time step), the ElevatorGUI object calls the ElevatorGraphics object to
     * perform an animation step, i.t. to inspect the status of the Model (all
     * elevators: motors, doors and scales) and to display a new state (by
     * repainting the ElevatorCanvas) if the state has changed since the
     * previous stemp.
     */
    ...
    /**
     * Creates a graphical user interface of the Elevator Application, creates
     * an object with the ElevatorEvents class to listen for action events from
     * buttons and to print action commands carried in the events, creates an
     * object with the ElevatorGraphics used to animate elevators, creates an
     * array of objects with the ElevatorCanvas class used to display elevators,
     * creates and starts the Timer object that clocks animation in the
     * ElevatorGraphics object, creates a object with the ElevatorIO class to
     * handle application input/output via standard input/output or TCP sockets
     * or/and Java RMI in separate threads.
     */
    ...
    /**
     * Invoked from the Timer object (that services as a clock) with a
     * predefined rate controlled with a JSlider guage
     */
     ...
     * computes and sets a new rate to the Timer and restarts the Timer with the
     * new rate. Invoked when the Timer rate is changed with the JSlider gauge.
     */

 ...
 */
/**
 * Provides input and execution of control commands (open/close doors, star/stop
 * moving elevators, set/get values of scales) from standard input, TCP socket
 * connections or/and via Java RMI (from servants), and output of action
 * commands from the floor and inside buttons and current position of elevators
 * to standard output, TCP socket or/and via Java RMI (to listening servants).
 */
public class ElevatorIO extends Thread {
    ...
    /**
     * The buffering character-input stream (BufferedReader) for reading char
     * and String from the input stream of the standard "input" or a TCP socket.
     * Used for reading control commands. Initialized to BufferedReader for
     * reading from the standard input
     */
    protected static BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
    /**
     * The print stream for printing action commands from buttons and current
     * positions of elevators to the output stream of the standard "output" or a
     * TCP socket. Initialized to PrintStream for printing to the standard
     * output
     */
     ...
     * a client (a controller) connects if Elevators has started with the "-tcp"
     * option, it creates a GetAllImpl object for controlling Elevators via Java
     * RMI and binds its reference (stub) to the "GetAll" name at the
     * rmiregistry if Elevators has started with the "-rmi" option.
     *
     * @param elevators is a reference to the object of the Elevators primary
     * class used to access the state of the Elevator objects (the Model)
     */
    public ElevatorIO(Elevators elevators) {
        super();
        this.allElevators = elevators.allElevators;
        if (Elevators.tcp) {
            connectToClient(Elevators.inPort, Elevators.outPort);
        }
        if (Elevators.rmi) {
            try {
    ...
    /**
     * Opens a TCP socket or a couple of sockets (if different ports are
     * specified for input and output) and waits until a client(s) connects.
     *
    ...
    public void connectToClient(int inPort, int outPort) {
        ...
        if (c2 != null) {
            ...
            out = new PrintStream(c1.getOutputStream(), true);
        }
    }

    /**
     * Reads and execute control commands from standard or socket (if open)
     * input stream in a separate thread until the thread is closed ot the
     * "quit" ("q") command is read from the stream.
     */
    public void run() {
    ...
    /**
     * Reads elevator commands from the standard or the socket (if open) input
     * stream and executes over and over again until the input stream is closed
     * or the "quit" is read from the stream
     */
    ...
    /**
     * Executes a "m" ("motor") control command read from standard or socket
     * input stream: starts the elevator moving upwards or downwards, or stops
     * the motor.
     *
     * @param number the integer number of the motor (elevator) to start moving
     * ot to stop
     * @param val the integer code of a motor command: "1" - start moving
     * upwards, "0" - stop, "-1" - start moving downwards.
     */
    ...
    /**
     * Executes a "s" ("scale") control command read from standard or socket
     * input stream: sets a given value to the elevator scale.
     *
     * @param number the integer number of the elevator whose scale to set
     * @param val the integer value to be set to the scale of the given
     * elevator.
     */
    // used by TCP and standard IO
    ...
    /**
     * Executes a "d" ("door") control command read from standard or socket
     * input stream: starts/stops openning/closing the door.
     *
     * @param number the number of the elevator whose door to open/close
     * @param val the integer code of a door command: "1" - open, "-1" -close,
     * "0" - stop
     */
    // used by TCP and standard IO
    ...
    /**
     * Executes a "w" ("where") control command read from standard or socket
     * input stream: gets and prints the current position of the elevator to
     * standard or socket (if open) input stream. The position is printed as "f
     * <i>n p</i>", where "f" stands for "floor units", <i>n</i> is the elevator
     * number and <i>p</i> is a floating point value of the position in "floor
     * units" (e.g. "f 2 2.5" means that the 2nd elevator is exactly in between
     ...
     * Called by an RMI servant that implements one of these interface. Starts
     * the elevator moving upwards or downwards, or stops the motor.
     *
     * @param number the integer number of the motor (elevator) to start moving
     * ot to stop
     * @param val the integer code of a motor command: "1" - start moving
     ...
     * Called by an RMI servant that implements one of these interface. Sets a
     * given value to the elevator scale.
     *
     * @param number the integer number of the elevator whose scale to set
     * @param val the integer value to be set to the scale of the given
     * elevator.
     ...
     * PositionListener objects (if any). Returns the current position of the
     * elevator to the calling thread.
     *
     * @param number the number of the elevator whose position to get
     * @return float position of the elevator in "floor units" (e.g. 2.5 "floor
     * units means that the elevator is exactly in between the 2nd and 3rd
     * floor)
     * @throws IllegalParamException if number is not a legal elevator number
     */
     ...
     * accepted on an remote interface used to control motors such as Motors and
     * Elevators. Called by an RMI servant that implements one of these
     * interface and by PositionListener objects (if any). Returns current
     * positions of all elevators to the calling thread.
     *
     * @return an array of floatint point values of positions of the elevator in
     * "floor units" (e.g. 2.5 "floor units means that the elevator is exactly
     * in between the 2nd and 3rd floor)
     */
    ...
    /**
     * Returns a current value of a scale. Called by an RMI servant that
     * implements one of the interfaces used to control scales such as Scale,
     * Scales, Elevator and Elevators. Returns current value of the scale.
     *
 ...
 * output stream for printing action commands from elevator and floor buttons
 * and current positions of elevators.
 */
class Connector extends Thread {
     ...
     *
     * @return an input stream for reading bytes from the socket of this
     * Connector.
     */
     ...
     *
     * @return an output stream for writing bytes to the socket of this
     * Connector.
     */

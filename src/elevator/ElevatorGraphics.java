 ...
 */
/**
 * Provides animation of elevators at a rate of the amination Timer. The
 * <code>animate</code> or the <code>aminateAll</code> methods are called at the
 * rate set to the animation Timer of the ElevatorGUI object. The methods check
 * whether the state of the elevator(s) Model has changed and if so, requests
 * the ElevatorCanvas object(s) to repaint the elevator(s) with the new state.
 * How to animate, one elevator at a time step (with the animate method) or all
 * elevators at a time step (with the animateAll method) is controlled by a
 * "hard-coded" boolean variable <code>animateAllAtOnce</code> defined in the
 * ElevatorGUI class. Maybe this should be controlled with a command line option
 * (to be fixed). Now the option is set to true that means that the animateAll
 * methos is called each time step to update the view of ALL elevators rather
 * than one at a time.
 */
public class ElevatorGraphics {
    ...
    /**
     * Creates an object with the ElevatorGraphics class, gets a reference to
     * array of Elevator objects (the model of elevators) and the ElevatorIO
     * object.
     *
    ...
    /**
     * Gets a current state of one elevator and compares with the old (previous)
     * state, requests to repaint the elevator if the old state has changed,
     * stores the new state as the old state. Invoked on an event from the
     * application Timer of the ElevatorGUI object.
     *
    ...
    public void animate(int number) { // only elevator with the current number
        ...
        if (doordir != Elevators.STOP || boxdir != Elevators.STOP) {
            presentation(number);
        }
    ...
    /**
     * Gets a current state of all elevator (one by one) and compare with the
     * old (previous) state, requests to repaint the elevator if the old state
     * has changed, stores the new state as the old state. Invoked on an event
     * from the application Timer of the ElevatorGUI object.
     *
     * @param number is the elevator number to be animated
     * @see #animate(int)
     */
     ...
     * according to its current (new) state, prints a current position of the
     * elevator to the output stream (standard or socket)
     */
     ...
     *
     * @param number the integer number of the elevator whose scale value to
     * present on the JProgressBar
     */
